...
#define IDGEN_LONG_ID_TYPE 1             // idgen key is xxx@l long id type  
#define IDGEN_SHORT_ID_TYPE 2            // idgen key is xxx@s short id type
#define IDGEN_CONTINUOUS_ID_TYPE 3       // idgen key is xxx@c continues id type
#define IDGEN_QUERY_TYPE 4               // idgen key is xxx@l#  query xxx@l (etc. @s @c)
#define IDGEN_NORMAL_TYPE 5              // idgen key is other format

#define IDGEN_LONG_ID_SUFFIX "@l"
#define IDGEN_SHORT_ID_SUFFIX "@s"
#define IDGEN_CONTINUOUS_ID_SUFFIX "@c"
#define IDGEN_QUERY_SUFFIX "#"

#define IDGEN_QUERY_SUFFIX_LEN (sizeof(IDGEN_QUERY_SUFFIX) - 1)

#define IDGEN_TIME_SHIFT 10000000000U
#define IDGEN_SERVER_NUM_SHIFT 100000000U
#define IDGEN_SHORT_ID_MAX 99999999U
#define IDGEN_CONTINUOUS_ID_MAX 4000000000U

#define IDGEN_MAX_VSIZ	32

/* server number for idgen server. */
int server_num = 0;
...
static void usage(void){
  ...
  fprintf(stderr, "  %s [-g server_num(>0)][-host name] [-port num] [-thnum num] [-tout num]"
...
/* get idgen key type */
static int idgen_type(const char *key)
{
  if (tcstrbwm(key, IDGEN_LONG_ID_SUFFIX)) {
    return IDGEN_LONG_ID_TYPE;
  } else if (tcstrbwm(key, IDGEN_SHORT_ID_SUFFIX)) {
    return IDGEN_SHORT_ID_TYPE;
  } else if (tcstrbwm(key, IDGEN_CONTINUOUS_ID_SUFFIX)) {
    return IDGEN_CONTINUOUS_ID_TYPE;
  } else if (tcstrbwm(key, IDGEN_QUERY_SUFFIX)){
    return IDGEN_QUERY_TYPE;
  } else {
    return IDGEN_NORMAL_TYPE;
  }
}

/* idgen get, incr value in tc, return new value */
static char* idgen_get(TCULOG *ulog, uint32_t sid, uint32_t mid, TCADB *adb, const char *kbuf, int ksiz, int *vsizp)
{
  uint64_t value = 0;
  char *vbuf = NULL;
  char *res = NULL;
  switch(idgen_type(kbuf))
  {
    case IDGEN_LONG_ID_TYPE:
      vbuf = (char *)tcadbget(adb, kbuf, ksiz, vsizp);
      if (!vbuf) {
        value = (uint64_t) time(NULL) * IDGEN_TIME_SHIFT + (uint64_t)server_num * IDGEN_SERVER_NUM_SHIFT; 
        ttservlog(g_serv, TTLOGINFO, "IDGEN_LONG_ID_TYPE init key:%s value:%"PRIu64, kbuf, value);
      } else {
        uint64_t val;
        sscanf(vbuf, "%"PRIu64, &val);
        value = ((uint64_t)val + 1) % IDGEN_SERVER_NUM_SHIFT + (uint64_t) time(NULL) * IDGEN_TIME_SHIFT + (uint64_t)server_num * IDGEN_SERVER_NUM_SHIFT;
        tcfree(vbuf);
      }

      res = tcmalloc(IDGEN_MAX_VSIZ);
      sprintf(res, "%020"PRIu64, value);
      *vsizp = strlen(res);
      if(!tculogadbput(ulog, sid, 0, adb, kbuf, ksiz, res, *vsizp)){
        ttservlog(g_serv, TTLOGERROR, "do_idgen_get: operation failed");
				return NULL;
      }
      return res;
    case IDGEN_SHORT_ID_TYPE:
      vbuf = (char *)tcadbget(adb, kbuf, ksiz, vsizp);
      if (!vbuf) {
        value = (uint64_t)server_num * IDGEN_SERVER_NUM_SHIFT;
        ttservlog(g_serv, TTLOGINFO, "IDGEN_SHORT_ID_TYPE init key:%s value:%"PRIu64, kbuf, value);
      } else {
        uint64_t val;
        sscanf(vbuf, "%"PRIu64, &val);
        if (val % IDGEN_SERVER_NUM_SHIFT == IDGEN_SHORT_ID_MAX) {
          ttservlog(g_serv, TTLOGERROR, "short id reach maxid %"PRIu64, val);
          tcfree(vbuf);
          return NULL;
        }
        value = ((uint64_t)val + 1) % IDGEN_SERVER_NUM_SHIFT + (uint64_t)server_num * IDGEN_SERVER_NUM_SHIFT;
        tcfree(vbuf);
      }

      res = tcmalloc(IDGEN_MAX_VSIZ);
      sprintf(res, "%010"PRIu64, value);
      *vsizp = strlen(res);
      if(!tculogadbput(ulog, sid, 0, adb, kbuf, ksiz, res, *vsizp)){
        ttservlog(g_serv, TTLOGERROR, "do_idgen_get: operation failed");
				return NULL;
      ...
      return res;
    case IDGEN_CONTINUOUS_ID_TYPE:
      vbuf = (char *)tcadbget(adb, kbuf, ksiz, vsizp);
      if (!vbuf) {
        value = 1U;
        ttservlog(g_serv, TTLOGINFO, "IDGEN_CONTINUOUS_ID_TYPE init key:%s value:%"PRIu64, kbuf, value);
      } else {
        uint64_t val;
        sscanf(vbuf, "%"PRIu64, &val);
        if (val == IDGEN_CONTINUOUS_ID_MAX) {
          ttservlog(g_serv, TTLOGERROR, "continuous id reach maxid %"PRIu64, val);
          tcfree(vbuf);
          return NULL;
        }
        value = val + 1;
      ...
      *vsizp = strlen(res);
      ...
      return res;
    case IDGEN_QUERY_TYPE:
      return tcadbget(adb, kbuf, ksiz - IDGEN_QUERY_SUFFIX_LEN, vsizp);
    case IDGEN_NORMAL_TYPE:
      return tcadbget(adb, kbuf, ksiz, vsizp);
    default:
      break;
  }
  return NULL;
  ...
  for(int i = 1; i < tnum; i++){
    ...
    if(mask & ((1ULL << TTSEQGET) | (1ULL << TTSEQALLMC) | (1ULL << TTSEQALLREAD))){
      ...
      if(pthread_mutex_lock(rmtxs + mtxidx) != 0){
        ...
        ttservlog(g_serv, TTLOGERROR, "do_mc_get: pthread_mutex_lock failed"); 
        return;
      }
      vbuf = idgen_get(ulog, sid, 0, adb, kbuf, ksiz, &vsiz); 
      if(vbuf){
      ...
      if(pthread_mutex_unlock(rmtxs + mtxidx) != 0) {
        ttservlog(g_serv, TTLOGERROR, "do_mc_get: pthread_mutex_unlock failed");
      }
    }
  }
